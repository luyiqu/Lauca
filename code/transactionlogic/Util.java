...
public class Util {
	...
	public static boolean isMonotonous(Object obj1, Object obj2, int dataType, double[] increment) {
		...
		case 3:
			...
			break;
		case 1:
			double dtmp = (Double) obj2 - (Double) obj1;
			if (increment[0] == Double.MIN_VALUE) {
				increment[0] = dtmp;
				return true;
			} else {
	 ...
	 * @return formattedCounter：格式化后的计数器，以方便后续的关联关系分析
	...
	public static List<Entry<String, List<Entry<String, Double>>>> convertCounter(
		...
		Map<String, List<Entry<String, Double>>> map4FormattedCounter = new HashMap<>();

		for (Entry<String, Map<String, Integer>> entry : counter.entrySet()) {
			// entry为当前参数的等于关联关系统计信息，下面需将其中的事务实例个数转化成比例
			Map<String, Double> entryFormattedValueMap = new HashMap<>();
			...
			for (Entry<String, Integer> tmp : entry.getValue().entrySet()) {
				// entryFormattedValueMap.put(tmp.getKey(), tmp.getValue().doubleValue() / executionNum);
				...
				if (probability >= Configurations.getMinProbability()) {
					entryFormattedValueMap.put(tmp.getKey(), probability);
				}

			}

			// Entry<>对象无法自己new出来，所以只能这么迂回处理。下面的entryFormattedValueList是针对一个输入参数的格式化后的统计信息
			List<Entry<String, Double>> entryFormattedValueList = new ArrayList<>(entryFormattedValueMap.entrySet());

			// 排序规则：关联关系强弱，逆序 -> 操作的前后顺序，升序 -> 对象类型（para or result），先参数后返回结果集元素 -> 对象的index
			entryFormattedValueList.sort(new EntryFormattedValueComparator());
			map4FormattedCounter.put(entry.getKey(), entryFormattedValueList);
		}

		List<Entry<String, List<Entry<String, Double>>>> formattedCounter = new ArrayList<>(map4FormattedCounter.entrySet());
		// 排序规则：操作的前后顺序，升序 -> 参数的index，升序
		formattedCounter.sort(new ParaIdentifierComparator());

//		System.out.println("Util.convertCounter -> formattedCounter: \n\t" + formattedCounter);
		return formattedCounter;
	}
}

class EntryFormattedValueComparator implements Comparator<Entry<String, Double>> {
	private static final Pattern hyphenPattern = Pattern.compile("_");
	...
	public int compare(Entry<String, Double> o1, Entry<String, Double> o2) {
		...
		if (o1.getValue().compareTo(o2.getValue()) != 0 ){
			...
			int operationId1 = Integer.parseInt(hyphenPattern.split(o1.getKey())[0]);
			int operationId2 = Integer.parseInt(hyphenPattern.split(o2.getKey())[0]);
			if (operationId1 < operationId2) {
				...
				// type：para or result
				String type1 = hyphenPattern.split(o1.getKey())[1];
				String type2 = hyphenPattern.split(o2.getKey())[1];
...
class ParaIdentifierComparator implements Comparator<Entry<String, List<Entry<String, Double>>>> {
	...
	public int compare(Entry<String, List<Entry<String, Double>>> o1, Entry<String, List<Entry<String, Double>>> o2) {
		int operationId1 = Integer.parseInt(hyphenPattern.split(o1.getKey())[0]);
		int operationId2 = Integer.parseInt(hyphenPattern.split(o2.getKey())[0]);
		int paraIndex1 = Integer.parseInt(hyphenPattern.split(o1.getKey())[2]);
		int paraIndex2 = Integer.parseInt(hyphenPattern.split(o2.getKey())[2]);
		// 首先根据操作的前后顺序进行排序，升序
		if (operationId1 < operationId2) {
			return -1;
		} else if (operationId1 > operationId2) {
